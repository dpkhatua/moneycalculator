...
After that, **Backup to Drive** and **Restore from Drive** just work. You'll need to reconnect roughly every hour of continuous use (Google's access tokens expire) or after closing the browser — this is intentional, so no long-lived credential is ever stored on the device.
