...
// ---- Auto price fetch: CoinGecko (crypto, keyless) + Twelve Data (stocks, needs a free key) ----
const TWELVE_DATA_KEY_STORAGE = 'spendingTracker.twelveDataKey';
function loadTwelveDataKey(){ return localStorage.getItem(TWELVE_DATA_KEY_STORAGE) || ''; }
function saveTwelveDataKey(key){ localStorage.setItem(TWELVE_DATA_KEY_STORAGE, key); }
document.getElementById('twelveDataKeyInput').value = loadTwelveDataKey();
document.getElementById('saveTwelveDataKey').addEventListener('click', ()=>{
  saveTwelveDataKey(document.getElementById('twelveDataKeyInput').value.trim());
  document.getElementById('priceRefreshStatus').textContent = 'Twelve Data key saved.';
});

async function fetchAutoPrice(h){
  if(!h.ticker) return { skipped:true };
  if(h.assetClass==='crypto'){
    try{
      const vsCurrency = currentCurrency==='USD' ? 'usd' : 'inr';
      const res = await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${encodeURIComponent(h.ticker.toLowerCase())}&vs_currencies=${vsCurrency}`);
      if(!res.ok) return { error: 'HTTP '+res.status };
      const data = await res.json();
      const price = data[h.ticker.toLowerCase()] && data[h.ticker.toLowerCase()][vsCurrency];
      if(price==null) return { error: 'Coin ID not recognized by CoinGecko' };
      return { price };
    } catch(e){ return { error: 'Network/CORS error' }; }
  }
  if(h.assetClass==='usstock' || h.assetClass==='equity'){
    const key = loadTwelveDataKey();
    if(!key) return { error: 'No Twelve Data API key saved' };
    try{
      const res = await fetch(`https://api.twelvedata.com/price?symbol=${encodeURIComponent(h.ticker)}&apikey=${encodeURIComponent(key)}`);
      if(!res.ok) return { error: 'HTTP '+res.status };
      const data = await res.json();
      if(data.status==='error' || !data.price) return { error: data.message || 'Symbol not recognized' };
      return { price: +data.price };
    } catch(e){ return { error: 'Network/CORS error — this API may not allow direct browser requests' }; }
  }
  return { skipped:true }; // no known auto-source for this asset class
}
...
async function refreshHoldingPrice(id){
  ...
  const result = await fetchAutoPrice(h);
  if(result.price!=null){
    ...
    status.textContent = `Couldn't fetch ${h.name}: ${result.error||'no auto-source for this asset type'} — update it manually instead.`;
  }
}
...
document.getElementById('refreshAllPrices').addEventListener('click', async ()=>{
  ...
  saveData();
  renderNetWorth();
  status.textContent = `Updated ${updated} holding(s)${failed>0?`, ${failed} failed — update those manually`:''}.`;
});
