...
const ASSET_CLASS_LABELS = {
  savings:'Savings', fd:'FD', rd:'RD', equity:'Equity', mf:'Mutual Fund',
...
document.getElementById('addHolding').addEventListener('click', ()=>{
  ...
  const invested = +document.getElementById('holdingInvested').value;
  const currentValRaw = document.getElementById('holdingCurrent').value;
  const currentVal = currentValRaw === '' ? invested : +currentValRaw;

  if(!name){ alert('Give this holding a name.'); return; }
  if(!invested || invested<=0){ alert('Enter an invested amount greater than zero.'); return; }
  ...
  getNW().holdings.push({
    ...
    invested,
    current: currentVal,
    log: [{date: todayLocalISO(), type:'buy', amount: invested}]
  });
  saveData();
  document.getElementById('holdingName').value = '';
  document.getElementById('holdingInvested').value = '';
  document.getElementById('holdingCurrent').value = '';
  renderNetWorth();
});
...
function buyHolding(id){
  ...
  const amt = +raw;
  if(!raw || !amt || amt<=0) return;
  h.invested += amt;
  h.current += amt;
  h.log.push({date: todayLocalISO(), type:'buy', amount: amt});
  saveData();
  renderNetWorth();
}
function sellHolding(id){
  const h = getNW().holdings.find(x=>x.id===id);
  if(!h) return;
  const raw = prompt(`Sell from "${h.name}" — amount to reduce (at current value):`, '');
  const amt = +raw;
  if(!raw || !amt || amt<=0) return;
  if(amt > h.current){ alert('That\'s more than the current value of this holding.'); return; }
  // Reduce cost basis proportionally to how much of the current value is being sold.
  const proportion = amt / h.current;
  h.invested = Math.max(0, h.invested - h.invested*proportion);
  h.current = Math.max(0, h.current - amt);
  h.log.push({date: todayLocalISO(), type:'sell', amount: amt});
  if(h.current <= 0.01 && !confirm(`"${h.name}" is now fully sold. Keep it in the list (0 value) or remove it?\n\nOK = remove, Cancel = keep at zero`)){
    // keep at zero
  } else if(h.current <= 0.01){
...
function updateHoldingValue(id){
  const h = getNW().holdings.find(x=>x.id===id);
  if(!h) return;
  const raw = prompt(`Update current value of "${h.name}" (was ${fmtAmount(h.current)}):`, h.current);
  if(raw===null || raw==='') return;
  const val = +raw;
  if(isNaN(val) || val<0){ alert('Enter a valid amount.'); return; }
  h.current = val;
  h.log.push({date: todayLocalISO(), type:'mark', amount: val});
  saveData();
  renderNetWorth();
}
function deleteHolding(id){
  const h = getNW().holdings.find(x=>x.id===id);
  if(!h) return;
  if(!confirm(`Delete "${h.name}" entirely? This can't be undone.`)) return;
  getNW().holdings = getNW().holdings.filter(x=>x.id!==id);
  saveData();
  renderNetWorth();
}
...
function renderHoldingsList(){
  ...
  holdings.slice().sort((a,b)=>a.assetClass.localeCompare(b.assetClass)).forEach(h=>{
    ...
    row.innerHTML = `
      <div class="h-class">${escapeHtml(ASSET_CLASS_LABELS[h.assetClass]||h.assetClass)}</div>
      <div class="h-name">${escapeHtml(h.name)}</div>
      <div class="h-figure"><span class="lbl">Invested</span>${fmtAmount(h.invested)}</div>
      <div class="h-figure"><span class="lbl">Current</span>${fmtAmount(h.current)}</div>
      <div class="h-figure h-gain${gain<0?' loss':''}"><span class="lbl">Gain/Loss</span>${gain>=0?'+':''}${fmtAmount(gain)} (${gainPct.toFixed(1)}%)</div>
      <div class="h-actions">
        <button class="buy" title="Buy more" data-action="buy">＋</button>
        <button class="sell" title="Sell" data-action="sell">－</button>
        <button title="Update value" data-action="mark">✎</button>
        <button title="Delete" data-action="del">×</button>
      </div>
    `;
    row.querySelector('[data-action=buy]').addEventListener('click', ()=>buyHolding(h.id));
    row.querySelector('[data-action=sell]').addEventListener('click', ()=>sellHolding(h.id));
    row.querySelector('[data-action=mark]').addEventListener('click', ()=>updateHoldingValue(h.id));
    row.querySelector('[data-action=del]').addEventListener('click', ()=>deleteHolding(h.id));
...
function renderHoldingsChart(){
  const holdings = getNW().holdings;
  const byClass = {};
  holdings.forEach(h=>{ byClass[h.assetClass] = (byClass[h.assetClass]||0) + h.current; });
  const labels = Object.keys(byClass).map(k=>ASSET_CLASS_LABELS[k]||k);
  const data = Object.values(byClass);
  const palette = ['#1F6F50','#C98A2C','#A2452F','#4B5A50','#7a9e8f','#d9b06b','#c47a68','#8fa89d','#3d6b8a'];
  if(labels.length===0){
...
function renderNWSummary(investmentValue){
  const nw = getNW();
  if(investmentValue===undefined) investmentValue = nw.holdings.reduce((s,h)=>s+h.current,0);
...
function mergeNetWorthFromBackup(incomingNW){
  ...
  ['INR','USD'].forEach(cur=>{
    ...
    if(Array.isArray(incomingBucket.holdings)){
      ...
      incomingBucket.holdings.forEach(h=>{ if(h.id && !existingIds.has(h.id)){ localBucket.holdings.push(h); addedHoldings++; } });
    }
